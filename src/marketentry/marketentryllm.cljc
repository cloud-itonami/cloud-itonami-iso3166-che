(ns marketentry.marketentryllm
  ...
  the CHE public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ch-presence?=" (:has-ch-presence? e)
                        " uid-verified?=" (:uid-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ch-presence? e))
                              (:has-ch-presence? e))
                          (or (not (:requires-uid? e))
                              (:uid-verified? e)))
                   0.9 0.3)}))
