(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Switzerland-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Switzerland-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ch-presence?
                                       true` (near-universal for Switzerland
                                       ...
                                       Grounded in Swiss commercial
                                       registration (UID) typically required
                                       for federal procedures (contracting
                                       authorities / BKB).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(UID/統一資格/simap.ch登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ch-presence-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ch-presence? true`, INDEPENDENTLY verify
  `:has-ch-presence?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ch-presence?` ground truth (most Switzerland public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ch-presence? e))
                 (not (true? (:has-ch-presence? e))))
        [{:rule :ch-presence-missing
          :detail (str subject " はスイス居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- uid-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-uid? true`, INDEPENDENTLY check
  `:uid-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-uid? e))
                 (not (true? (:uid-verified? e))))
        [{:rule :uid-unverified
          :detail (str subject " はUID確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ch-presence-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (uid-unverified-violations request st)
