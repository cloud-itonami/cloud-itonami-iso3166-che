(ns marketentry.store
  "SSoT for the CHE market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems CH" :portal "simap.ch"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ch-presence? true :has-ch-presence? true
             :requires-uid? true :uid-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CHE" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "simap.ch"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ch-presence? true :has-ch-presence? true
             :requires-uid? true :uid-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "simap.ch"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "CHE" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "simap.ch"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ch-presence? true :has-ch-presence? false
             :requires-uid? true :uid-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CHE" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "simap.ch"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ch-presence? true :has-ch-presence? true
             :requires-uid? true :uid-verified? false
             :drafted? false :submitted? false
             :jurisdiction "CHE" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "simap.ch"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ch-presence? has-ch-presence?
                               requires-uid? uid-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ch-presence?)  (assoc :engagement/requires-ch-presence? requires-ch-presence?)
    (some? has-ch-presence?)       (assoc :engagement/has-ch-presence? has-ch-presence?)
    (some? requires-uid?)    (assoc :engagement/requires-uid? requires-uid?)
    (some? uid-verified?)    (assoc :engagement/uid-verified? uid-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ch-presence? :engagement/has-ch-presence?
   :engagement/requires-uid? :engagement/uid-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ch-presence? (boolean (:engagement/requires-ch-presence? m))
     :has-ch-presence? (boolean (:engagement/has-ch-presence? m))
     :requires-uid? (boolean (:engagement/requires-uid? m))
     :uid-verified? (boolean (:engagement/uid-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-che"})
