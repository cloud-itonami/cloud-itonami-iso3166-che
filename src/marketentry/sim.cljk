...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (CHE, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ch-presence-missing) ==")
    ...
    (println "== filing/submit eng-4 (ch-presence-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up uid-unverified) ==")
    ...
    (println "== filing/submit eng-5 (uid-unverified -> HARD hold) ==")
